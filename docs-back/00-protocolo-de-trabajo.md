...
**El entregable para Ale es `docs-back/12-contrato-front.md`**: pantalla por pantalla, con el objeto exacto que recibe cada una, para que pueda construir contra el contrato **sin esperar al backend** y repartir el trabajo como le convenga. `T12` queda como la versión técnica, con el porqué de cada cambio.

**El consejo que decide cuánto duele conectar después**: que el mock ya devuelva la forma final de la respuesta. Si lo hace, conectar es cambiar de dónde sale el dato; si no, hay que reescribir cada pantalla dos veces.
